...
model_name = 'models/trained/ResNet50_FC_localization_20_[100]_[0]_3e-05_1863.72.hdf5'
...
for im, bbox, bbox_pred in zip(x,y,y_pred):
    ...
    draw_bbox.rectangle( (bbox[0], bbox[1], bbox[0]+bbox[2], bbox[1]+bbox[3]), outline="red")

    draw_pred_bbox = ImageDraw.Draw(im)
    draw_pred_bbox.rectangle( (bbox_pred[0], bbox_pred[1], bbox_pred[0]+bbox_pred[2], bbox_pred[1]+bbox_pred[3]), outline="green")

    print '350x350', (bbox[0], bbox[1], bbox[0]+bbox[2], bbox[1]+bbox[3]), (bbox_pred[0], bbox_pred[1], bbox_pred[0]+bbox_pred[2], bbox_pred[1]+bbox_pred[3])
    imgs.append(im)
